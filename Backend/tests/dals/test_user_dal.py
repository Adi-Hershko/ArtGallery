...
@pytest.mark.asyncio
async def test_add_user_should_return_single_user_from_db(session: Session, db_teardown, db_operations):
    user = User(username='sample_username', password=b'sample_password', is_active=True)
    expected_user_from_db = User(username='sample_username', password=b'sample_password', is_active=True)
    ...
    result = new_session.query(User).all()

    assert len(result) == 1
    assert_user(expected_user_from_db, result[0])
